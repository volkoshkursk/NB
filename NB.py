...
def train(C, D, D_c):
	...
	array[:] = [s.encode() for s in D_c[1]]
	for i in C.keys():
		prior.update(dict.fromkeys([i],mi.count(array, len(D_c[1]),create_string_buffer(str.encode('|' + i + '|')))/ N)) 
#		text = C[i] 
		mi.count_arr.restype = py_object
#----------------------------------------
		text = ''
		text += ' '.join([D_c[0][s] for s in mi.count_arr(array, len(D_c[1]), create_string_buffer(str.encode('|' + i + '|')))]).lower()
		text = text.split(' ')
#----------------------------------------
...
def use(C, prior, condprob, d):
	body = list()
	if d.body != None:
		body = d.body.lower().translate(str.maketrans(',:"0123456789.()', '                ', "';\/<>-")).split() # получить тела, сделать все буквы строчными, заменить лишние символы пробелами и разделить на слова (по стандартному алгоритму)
	title = list()
	if d.title != None:
...
#	for i in cat[num]:
#		cursor.execute("select word from " + groupname[num] + " where classname== '" + i + "' order by mi desc limit 10")
...
#	print(use(C, prior, condprob, D_test[0]))
#	return
'''
