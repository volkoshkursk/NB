...
def create(classname, arr, cursor, conn, groupname):
	if len(arr) == 0:
		return
	command = "INSERT into " + groupname + " values ("+ "'" + classname +"','" + arr[0] + "','" + str(arr[1]) + "'); \n"
	try:
		cursor.execute(command)
	except sqlite3.DatabaseError as e:
		print(e)
...
def main_mi(num):
	...
	for i in all_arr:
		body = list()
		if i.body != None:
			body = i.body.lower().translate(str.maketrans(',:"0123456789.()', '                ', "';\/<>-")).split() # получить тела, сделать все буквы строчными, заменить лишние символы пробелами и разделить на слова (по стандартному алгоритму)
...
#		class_arr = []
		for j in words:
			mi_v = mi.mi(array, len(all_arr), create_string_buffer(str.encode('|' + arr[i]+ '|')), array1, create_string_buffer(str.encode(j)))
#			print(mi_v)
			if mi_v != -1:
...
def test():
	...
	mi.mi.restype = c_double
	mi.mi(array, len(all_arr), create_string_buffer(str.encode('|ipe|')), array1, create_string_buffer(str.encode('exist')))
